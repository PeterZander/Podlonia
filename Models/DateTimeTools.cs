...
{
    public static class DateTimeRFC822
    {
        private static DateTime? ParseDate822( string date )
        {
            ...
            if ( DateTime.TryParseExact( date,
                new string[] { FORMAT, FORMAT2, FORMAT3, FORMAT4 },
        ...
        public static DateTime? TryToParseDateTime( string dttxt )
        {
            var try1 = ParseDate822( dttxt );
            if ( try1 != null ) return try1;

            bool triedreplace = false;
        again:
            try
            {
            ...
            catch ( System.FormatException ex )
            {
                ...
                if ( !triedreplace )
                {
                    foreach ( var tz in TimeZones ) dttxt = dttxt.Replace( tz.Key, tz.Value );
                    triedreplace = true;
                    goto again;
                }
